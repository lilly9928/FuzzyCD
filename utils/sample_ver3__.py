...
def ts_output(x_range,x1_val, x_vals, memfun, next_token_logits, next_token_logits_cds, alpha=0.7):
    low, med, high = memfun
    ...
    for (total_activation, rules), logits_cd in zip(activations, next_token_logits_cds):
        ...
        weight1 = rules[1] + rules[2] + rules[5]  # x1_val과 관련된 규칙 활성화도 confidence score 가 low일때 
        weight2 = rules[3] + rules[6] + rules[7]  # x2_val과 관련된 규칙 활성화도 confidence score 가 low일때  
        weight3 = rules[0] + rules[4] + rules[8]  # 두 값이 함께 높거나 낮은 경우의 규칙 활성화도

        # import pdb;pdb.set_trace()
        # weight1 = rules[0] + rules[1] + rules[2]  # x1_val과 관련된 규칙 활성화도 confidence score 가 low일때 
        # weight2 = rules[3] + rules[4] + rules[5]  # x2_val과 관련된 규칙 활성화도 confidence score 가 low일때  
        # weight3 = rules[6] + rules[7] + rules[8]  # 두 값이 함께 높거나 낮은 경우의 규칙 활성화도
    ...
    combined_logits= combined_probs
    ...
    js_div = jensen_shannon_divergence(torch.softmax(next_token_logits, dim=0), torch.softmax(combined_logits, dim=0))
    ...
    adjusted_logits = (1 + js_div) * p - js_div * combined_logits
    ...
    **model_kwargs,
    ...
    while True:
        ...
        if membership_init:   
            # next_token_logits = outputs.logits[:, -1, :] 
            # import pdb;pdb.set_trace()
            top1= torch.max(next_token_logits)
            return top1
        ...
        if use_cd:
            ...
            if use_FLL:
                ...
                adjusted_logits = ts_output(x_range,x1_val, x_vals, memfun, next_token_logits, next_token_logits_cds, alpha)
