...
def ts_output(x_range_top, memfun_top, top10, next_token_logits, next_token_logits_cds, alpha=0.7):
    ...
    f_top10,token=torch.topk(next_token_logits_cds,10)
    ts_output_debug['filter_top10_token'].append(token.cpu().tolist())
    ...
    if rule3:  # 원본 변동성이 적은 경우 할루시네이션이 아님 
        cd_alpha = 0
    elif rule4: # top2 값이 더 적은 경우 할루시네이션이 아님 
        cd_alpha = 0
    ...
    **model_kwargs,
    ...
    while True:
        ...
        if membership_init:   
            # next_token_logits = outputs.logits[:, -1, :] 
            # import pdb;pdb.set_trace()
            top10, _=torch.topk(next_token_logits,10)
            
            return top10.cpu().tolist()
