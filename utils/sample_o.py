...
def ts_output(x_range, x_vals, memfun, next_token_logits, next_token_logits_cds):
    low, med, high = memfun
    ...
    for (total_activation, rules), logits_cd in zip(activations, next_token_logits_cds):
        ...
        combined_probs += (0.7 * p + 0.3 * (weight1 * p + weight2 * q + weight3 * (p + q) / 2) / (total_activation + 1e-9))
        # print(p)
...
def sample(
    ...
    **model_kwargs,
    ...
    while True:
        ...
        use_cd = model_kwargs.get("images_1") != None
        ...
        if use_cd:
            ...
            model_inputs_1 = self.prepare_inputs_for_generation_1(input_ids, **model_kwargs_cd)
            model_inputs_2 = self.prepare_inputs_for_generation_2(input_ids, **model_kwargs_cd)
            model_inputs_3 = self.prepare_inputs_for_generation_3(input_ids, **model_kwargs_cd)
            model_inputs_4 = self.prepare_inputs_for_generation_4(input_ids, **model_kwargs_cd)
            ...
            adjusted_logits = ts_output(x_range, x_vals, memfun, next_token_logits, next_token_logits_cds)         
