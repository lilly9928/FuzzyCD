...
def sample(
    ...
    **model_kwargs,
    ...
    model_kwargs_cd_1 = model_kwargs.copy() # copy model_kwargs for cd only for the first forward process
    model_kwargs_cd_2 = model_kwargs.copy() # copy model_kwargs for cd only for the first forward process
    model_kwargs_cd_3 = model_kwargs.copy() # copy model_kwargs for cd only for the first forward process
    model_kwargs_cd_4 = model_kwargs.copy() # copy model_kwargs for cd only for the first forward process
    ...
    while True:
        ...
        # model_inputs['inputs_embeds'].shape, model_inputs['position_ids'].shape, model_inputs['past_key_values'].shape, model_inputs['use_cache'].shape, model_inputs['attention_mask'].shape,
        # model_inputs['inputs_embeds'].shape, model_inputs['position_ids'].shape, model_inputs['attention_mask'].shape,
        # model_inputs['input_ids'].shape, model_inputs['position_ids'].shape, model_inputs['attention_mask'].shape,
        # model_inputs_1['inputs_embeds'].shape, model_inputs_1['position_ids'].shape, model_inputs_1['attention_mask'].shape,
        # model_inputs_1['input_ids'].shape, model_inputs_1['position_ids'].shape, model_inputs_1['attention_mask'].shape,
        ...
        if use_cd:
            ...
            if use_FLL:
                ...
                model_inputs_1 = self.prepare_inputs_for_generation_f(input_ids, img_num=1,  **model_kwargs_cd_1)
                model_inputs_2 = self.prepare_inputs_for_generation_f(input_ids, img_num=2, **model_kwargs_cd_2)
                model_inputs_3 = self.prepare_inputs_for_generation_f(input_ids, img_num=3, **model_kwargs_cd_3)
                model_inputs_4 = self.prepare_inputs_for_generation_f(input_ids, img_num=4, **model_kwargs_cd_4)
        ...
        elif next_tokens.ndim == 2:
            input_ids = torch.cat([input_ids, next_tokens], dim=-1)
        else:
            ValueError(f"Expected next_tokens.dim is 1 or 2. But got a {next_tokens.dim}.")
        ...
        if use_cd:
            if use_FLL:
                ...
                outputs_1, model_kwargs_cd_1, is_encoder_decoder=self.config.is_encoder_decoder
                )
                model_kwargs_cd_2 = self._update_model_kwargs_for_generation(
                    outputs_2, model_kwargs_cd_2, is_encoder_decoder=self.config.is_encoder_decoder
                )
                model_kwargs_cd_3 = self._update_model_kwargs_for_generation(
                    outputs_3, model_kwargs_cd_3, is_encoder_decoder=self.config.is_encoder_decoder
                )
                model_kwargs_cd_4 = self._update_model_kwargs_for_generation(
                    outputs_4, model_kwargs_cd_4, is_encoder_decoder=self.config.is_encoder_decoder
                )
            else:
