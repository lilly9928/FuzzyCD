...
for dataset_name in "${datasets[@]}"; do
  ...
  for type in "${types[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=2 python ./src/object_hallucination_vqa_llava_ver4.py \
      ...
      --answers-file ./output/test/llava15M_${dataset_name}_pope_${type}_answers_seed${seed}_testing_rule_version4_all_logits.jsonl \
      --cd_alpha $cd_alpha \
      --cd_beta $cd_beta \
      --noise_step $noise_step \
      --seed ${seed} \
      --debug-file llava15M_${dataset_name}_pope_${type}_debug_logits_rule_version4_all_logits.jsonl \
