...
for dataset_name in "${datasets[@]}"; do
  ...
  for type in "${types[@]}"; do
    ...
    for sigma in 200 500 800 999; do
      # Display which dataset, type, and sigma are currently running
      echo "Running with dataset: ${dataset_name}, type: ${type}, sigma: ${sigma}"
      
      # Run the Python command
      CUDA_VISIBLE_DEVICES=4 python ./src/object_hallucination_vqa_llava_ver2.py \
        ...
        --answers-file ./output/ablation/llava15M_${dataset_name}_pope_${type}_answers_sigma${sigma}_seed${seed}_cd.jsonl \
        --cd_alpha $cd_alpha \
        --cd_beta $cd_beta \
        --filter_type diffusion \
        --noise_step $noise_step \
        --seed ${seed} \
        --sigma ${sigma}

      # Print completion message for this specific combination
      echo "Completed dataset: ${dataset_name}, type: ${type}, sigma: ${sigma}"
    done
  done
done
