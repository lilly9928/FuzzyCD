...
from llavaNext.llava.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from llavaNext.llava.conversation import conv_templates, SeparatorStyle
from llavaNext.llava.model.builder import load_pretrained_model
from llavaNext.llava.utils import disable_torch_init
from llavaNext.llava.mm_utils import tokenizer_image_token, get_model_name_from_path, KeywordsStoppingCriteria,process_images
...
def eval_model(args):
    ...
    logits_means_mean = torch.tensor(-0.11494282404581706).to('cuda')
    logits_means_std = torch.tensor( 0.1685815116932136).to('cuda')
    logits_min = torch.tensor(-0.677734375).to('cuda')
    logits_max = torch.tensor(-0.0032958984375).to('cuda')
        ...
        if args.use_cd:
            ...
            for filter_image in filter_images:
                if "diffusion" in args.filter_type:
                    ...
                    # image_tensors.append(image_processor.preprocess(filter_image, return_tensors='pt')['pixel_values'][0])
                    fi_tensor = process_images([filter_image], image_processor, model.config)
                    fi_tensor = [_image.to(dtype=torch.float16, device='cuda') for _image in fi_tensor]
                    image_tensors.append(fi_tensor[0])
            image_tensors = torch.stack(image_tensors, dim=0)
        else:
            image_tensor_cd = None     
        stop_str = conv.sep if conv.sep_style != SeparatorStyle.TWO else conv.sep2
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filter_type", type=str, default='FLL+gussian', help="Filter type", choices=["FLL+gussian", "FLL+median", "FLL+sharpen", "FLL+sobel", "FLL+bilateral", "FLL+diffusion", "gaussian", "median", "sharpen", "sobel", "diffusion"])
