...
def membership_init(args,questions,model,tokenizer,image_processor):
    ...
    for line in tqdm(sampled_questions):
        ...
        with torch.inference_mode():
            ...
            top1=output_logits.cpu()
            logits_top1.append(top1.item())
            
            logits_top1_mean = np.mean(logits_top1)
            logits_top1_std = np.std(logits_top1,ddof=1)
            logits_min = np.min(logits_top1)
            logits_max = np.max(logits_top1)

    return logits_top1_mean, logits_top1_std, logits_min, logits_max
...
def eval_model(args):
    ...
    if args.filter_type.startswith('FLL'):
        args.use_FLL = True
        logits_top1_mean, logits_top1_std, logits_min, logits_max = membership_init(args,questions,model,tokenizer,image_processor)
    else:
    ...
    for line in tqdm(questions):
        ...
        with torch.inference_mode():
                output_ids,mynext_token_logits_list,next_token_logits_list = model.generate(
                    ...
                    top_k = args.top_k,
                    max_new_tokens = 5,
                    logits_mean = logits_top1_mean,
                    logits_std = logits_top1_std,
