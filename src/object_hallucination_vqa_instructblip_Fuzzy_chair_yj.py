...
def eval_model(args):
    ...
    for line in tqdm(sampled_questions):
        ...
        if args.use_cd:
            ...
            if args.use_FLL:
                filter_images = add_filter(args,image_file=image_file, image_processor=vis_processors)
            else:
                filter_images = add_filter_adjust_sigma(args,image_file=image_file, sigma=args.sigma, image_processor=vis_processors)
            for filter_image in filter_images:
                if "diffusion" in args.filter_type:
                    image_tensors.append(filter_image)
                else:
                    image_tensors.append(vis_processors["eval"](filter_image).unsqueeze(0).to(device))
