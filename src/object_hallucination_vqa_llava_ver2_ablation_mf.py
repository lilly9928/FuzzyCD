...
def membership_init(args,questions,model,tokenizer,image_processor):
    args.use_FLL = True
    sample_size = int(len(questions) * args.sampling_ratio) 
...
def eval_model(args):
    ...
    if args.filter_type.startswith('FLL'):
        ...
        mf_data = {
            "logits_means_mean":logits_means_mean,
            "logits_means_std":logits_means_std,
            "logits_min":logits_min,
            "logits_max":logits_max
        }
        
        with open(f"./output/membership_ab/{args.data}_membership_{args.sampling_ratio}.json", "w", encoding="utf-8") as f:
    ...
    for line in tqdm(questions):
        ...
        # logits_means_mean, logits_means_std, logits_min, logits_max 
