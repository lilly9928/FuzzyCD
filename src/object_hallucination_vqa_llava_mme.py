...
from utils.image_filter import add_filter, add_filter_adjust_sigma, add_filter_whoo
...
def eval_model(args):
    ...
    if args.filter_type.startswith('FLL'):
        args.use_FLL = True
        sample_size = int(len(questions) * 0.1)  # 전체 길이의 10%
        sampled_questions = questions.select(range(sample_size))
        ...
        for line in tqdm(sampled_questions):
            ...
            if args.use_cd:
                ...
                if args.use_FLL:
                    # filter_images = add_filter_whoo(args,image_file=image)
    ...
    for line in tqdm(questions):
        ...
        with torch.inference_mode():
                output_ids, mynext_token_logits_list, next_token_logits_list = model.generate(
                    input_ids,
                    attention_mask=attention_mask, 
                    pad_token_id=model.config.eos_token_id,
                    images = image_tensors.half().cuda(),
        ...
        category_data[category].append(f"{image_name}\t{question}\t{gt}\t{model_answer}\n")
        ...
        ans_file.flush()
    ans_file.close()
    output_dir = "/data3/KJE/code/WIL_DeepLearningProject_2/Hallu_MLLM/test_others/log/MME/LLaVA_15FLL/results/"
# /data3/KJE/code/WIL_DeepLearningProject_2/Hallu_MLLM/test_others/log/MME/LLaVA_15FLL
    try:
        for category, lines in category_data.items():
            category_file = os.path.join(output_dir, f"{category}.txt")
            with open(category_file, 'w', encoding='utf-8') as f:
                f.writelines(lines)
            print(f"Category file saved: {category_file}")
    except Exception as e:
        print(f"Error saving category TXT files: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filter_type", type=str, default='FLL+sharpen')
