...
def membership_init(args,questions,model,tokenizer,image_processor):
    ...
    for line in tqdm(sampled_questions):
        ...
        # conv.append_message(conv.roles[0], qs+"List only objects names.")
        ...
        # breakpoint()
        try:
            image = Image.open(f'/data3/KJE/code/WIL_DeepLearningProject_2/Hallu_MLLM/data/COCO/images/train2014/COCO_train2014_{image_file}')   
            # image = Image.open(f'/data3/KJE/code/WIL_DeepLearningProject_2/Hallu_MLLM/base/ICD/experiments/data/chair/chair-500/{image_file}')   
        except:
            image = Image.open(f'/data3/KJE/code/WIL_DeepLearningProject_2/Hallu_MLLM/data/COCO/images/val2014/COCO_val2014_{image_file}')   
        image_tensor = image_processor.preprocess(image, return_tensors='pt')['pixel_values'][0]

        if args.use_cd:
            image_tensors = [image_tensor]
            if args.use_FLL:
                filter_images = add_filter(args,image_file=image, image_processor=image_processor)
            else:
                filter_images = add_filter_adjust_sigma(args,image_file=image, sigma=args.sigma, image_processor=image_processor)
...
def eval_model(args):
    ...
    with open(question_file, "r", encoding="utf-8") as f:
        first_char = f.read(1)  # 첫 글자 확인
        f.seek(0)  # 파일 포인터 원래 위치로 이동

        if first_char == "[":  
            # ✅ JSON 형식이면 json.load() 사용
            questions = json.load(f)
        else:
            # ✅ JSONL 형식이면 한 줄씩 json.loads() 사용
    ...
    for line in tqdm(questions):
        ...
        conv = conv_templates[args.conv_mode].copy()
        # conv.append_message(conv.roles[0], qs+"Please answer this question with one word.")
        # conv.append_message(conv.roles[0], qs+"You are an object detector to recognize every different objects.")
        # conv.append_message(conv.roles[0], qs)
        conv.append_message(conv.roles[0], qs+"List only objects names.")
        conv.append_message(conv.roles[1], None)
        prompt = conv.get_prompt()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image-folder", type=str, default="/data3/KJE/code/WIL_DeepLearningProject_2/Hallu_MLLM/base/ICD/experiments/data/chair/chair-500")
