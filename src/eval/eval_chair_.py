...
# nltk.data.path.append("/home/yoonji/nltk_data")
...
class CHAIR(object):
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key):
        ...
        for i in tqdm.trange(len(caps)):
            ...
            hallucinated = False
            hallucinated_word_count_before = hallucinated_word_count
            hallu_word = []
            # add
            ...
            for word, node_word, idx in zip(words, node_words, idxs):
                if node_word not in gt_objects:
                    ...
                    hallu_word.append((word, node_word))
            ...
            if hallucinated:
                #print(imid, hallucinated_word_count - hallucinated_word_count_before, hallu_word)
        ...
        F1_score = 2 * (1 - chair_i) * recall / (1 - chair_i + recall)
...
def load_generated_captions(cap_file, image_id_key:str, caption_key:str):
    ...
    # list of int
    imids = [int(obj[image_id_key].split('.')[0]) for obj in caps]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--caption_key", type=str, default="answer",
    ...
    cap_dict = evaluator.compute_chair(args.cap_file, args.image_id_key, "answer") 
    ...
    # ✅ hallucinated 문장만 jsonl로 저장
    hallucinated_jsonl_output_path = os.path.join(
        '/data3/KJE/code/WIL_DeepLearningProject_2/Hallu_MLLM/src/output/find_chair',
        os.path.basename(args.cap_file.split('/')[-1])
    )

    # 원본 cap_file 불러오기
    ext = os.path.splitext(args.cap_file)[-1]
    if ext == '.json':
        orig_data = json.load(open(args.cap_file))
    elif ext == '.jsonl':
        orig_data = [json.loads(line) for line in open(args.cap_file)]
    else:
        raise ValueError(f"Unsupported cap_file extension: {ext}")

    # image_id ↔ image name 매핑 (필요 시 정제)
    imageid_to_full_entry = {
        int(item[args.image_id_key].split('.')[0]): item
    ...
    for sent in cap_dict["sentences"]:
        if sent["metrics"]["CHAIRs"] == 1:
            imid = sent["image_id"]
            orig = imageid_to_full_entry.get(imid)
            if orig:
                hallucinated_entries.append({
                    "image": orig["image"],
                    "answer": orig["answer"],
                    "hallucinated_words": sent["mscoco_hallucinated_words"],
                    "gt_objects": sent["mscoco_gt_words"],              # ✅ GT 객체
                    "generated_objects": sent["mscoco_generated_words"] # ✅ 생성된 객체
                })


    with open(hallucinated_jsonl_output_path, "w") as f:
        for entry in hallucinated_entries:
            f.write(json.dumps(entry, ensure_ascii=False) + "\n")

    print(f"✅ Saved hallucinated entries to {hallucinated_jsonl_output_path}")
